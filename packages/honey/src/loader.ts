...
async function loader(
  ...
  if (!theme) {
    throw new Error('No Honey theme found!')
  }
  ...
  const prefix =
    `import __honey_withLayout__ from '${layout}'\n` +
    `import { withSSG as __honey_withSSG__ } from '@honeydocs/honey/ssg'\n` +
    `${layoutConfig
      ? `import __honey_layoutConfig__ from '${layoutConfig}'`
      : ''
    }\n\n` +
    `const __honey_pageMap__ = ${JSON.stringify(pageMap)}\n` +
    `globalThis.__honey_internal__ = {\n` +
    `  pageMap: __honey_pageMap__,\n` +
    `  route: ${JSON.stringify(route)},\n` +
    `}\n` +
    `
    const __honey_content__ = <MDXContent/>
    const HoneyLayout = __honey_withSSG__(__honey_withLayout__({
    ...
    }, ${layoutConfig ? '__honey_layoutConfig__' : 'null'}))
    `

  const suffix = `export default function HoneyPage (props) {
  return <HoneyLayout {...props}>{__honey_content__}</HoneyLayout>
}
HoneyPage.getLayout = HoneyLayout.getLayout`
