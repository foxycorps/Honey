...
function getContext(name: string) {
  if (!(globalThis as any).__honey_internal__) {
    throw new Error(
      `Honey context not found. Please make sure you are using "${name}" of "@honeydocs/honey/context" on a Honey page.`
    )
  }
  return (globalThis as any).__honey_internal__
}
