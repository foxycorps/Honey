...
const Item = ({
  ...
  return (
    <>
      {first ? (
        <div className="honey-search-section mx-2.5 px-2.5 pb-1.5 mb-2 mt-6 first:mt-0 font-semibold uppercase text-xs text-gray-500 select-none dark:text-gray-300">
          {page}
        </div>
      ) : null}
...
export default function Search() {
      ...
      switch (e.key) {
        case 'ArrowDown': {
          ...
          if (active + 1 < results.length) {
            ...
            const activeElement = document.querySelector(
              `.honey-flexsearch ul > a:nth-of-type(${active + 2})`
            )
        ...
        case 'ArrowUp': {
          ...
          if (active - 1 >= 0) {
            setActive(active - 1)
            const activeElement = document.querySelector(
              `.honey-flexsearch ul > a:nth-of-type(${active})`
            )
            if (activeElement && activeElement.scrollIntoView) {
              activeElement.scrollIntoView({
                behavior: 'smooth',
  ...
  const load = async () => {
    ...
    if (!indexes[localeCode] && !loading) {
      ...
      const data = await (
        await fetch(
          `${Router.basePath}/_next/static/chunks/honey-data-${localeCode}.json`
        )
      ).json()
  ...
  return (
    <div className="relative w-full honey-search honey-flexsearch md:w-64">
      {renderList && (
