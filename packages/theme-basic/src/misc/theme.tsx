...
const getComponents = (context: { index: number }, customComponents?: any) => {
  return ({
    a: A,
    table: Table,
    details: Details,
    summary: Summary,
    ...customComponents, // put it here, because we don't allow for headings to be overridden
    h2: H2(context),
    h3: H3(context),
    h4: H4(context),
    h5: H5(context),
...
export const MDXTheme: React.FC<{}> = ({ children }) => {
  ...
  return (
    <MDXProvider components={getComponents({ index: 0 }, customComponents) as any}>
      {children}
    </MDXProvider>
  )
}
