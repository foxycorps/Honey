...
export default function Sidebar({
  ...
  useEffect(() => {
    const activeElement = document.querySelector('.honey-sidebar li.active')
    ...
    if (activeElement) {
      scrollIntoView(activeElement, {
        ...
        inline: 'center',
        scrollMode: 'always',
        boundary: document.querySelector('.honey-sidebar-container')
      })
    }
  }, [])
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          'honey-sidebar-container honey-scrollbar fixed flex-shrink-0 w-full md:w-64 md:sticky z-[15] top-16 self-start overflow-y-auto transform-none h-[calc(100vh-4rem)]',
          asPopover ? 'md:hidden' : 'md:block',
          hasMenu ? 'with-menu' : '',
          { open: menu }
        )}
      >
        <div className="honey-sidebar select-none w-full h-full md:h-auto pl-[calc(env(safe-area-inset-left)-1.5rem)]">
          <div className="p-4 min-h-[calc(100vh-4rem-61px)]">
            <div className="honey-sidebar-search mb-4 block md:hidden">
              {config.customSearch ||
                (config.search ? (
          ...
          {!hasMenu ? null : (
            <div className="honey-sidebar-menu mx-4 border-t dark:border-neutral-800 shadow-[0_-12px_16px_white] dark:shadow-[0_-12px_16px_#111]">
