...
import { Heading, PageMapItem, PageOpt } from '@honeydocs/honey'
  ...
  return (
    <React.Fragment>
      ...
      {themeContext.layout === 'full' ? (
        <article className="honey-body full relative justify-center overflow-x-hidden pl-[max(env(safe-area-inset-left),1.5rem)] pr-[max(env(safe-area-inset-right),1.5rem)]">
        ...
        <div className="honey-body full relative overflow-x-hidden expand">
          {children}
        </div>
      ) : (
        <article
          className={cn(
            'honey-body relative pb-8 w-full justify-center max-w-full flex min-w-0 pr-[calc(env(safe-area-inset-right)-1.5rem)]',
            themeContext.typesetting
              ? 'honey-body-typesetting-' + themeContext.typesetting
              : ''
          )}
        >
...
const Content: React.FC<LayoutProps> = ({
  ...
  return (
    <React.Fragment>
      ...
      >
        <div
          className={cn('honey-container main-container flex flex-col', {
            rtl: isRTL,
            'menu-active': menu
          })}
        >
              ...
              <div className="flex flex-1 w-full">
                ...
                {activeType === 'page' ||
                  ...
                  themeContext.layout === 'full' ? null : (
                    <div className="honey-toc w-64 hidden xl:block text-sm px-4 order-last flex-shrink-0" />
                  )
                ) : (
                  <ToC
